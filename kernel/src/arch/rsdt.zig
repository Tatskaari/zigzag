...
pub const RSDT = extern struct {
    ...
    // findHdr searches for a SDT header in the RSDT
    pub fn findHdr(self: *RSDT, signature: [4]u8) !*Header {
        for(self.entries) |addr| {
...
pub fn getRsdt() *RSDT {
    ...
    const rsdt : *RSDT = @ptrFromInt(kernel.mem.hhdm.virtualFromPhysical(rsdp.rsdt));
