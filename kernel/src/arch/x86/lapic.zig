...
const APIC = packed struct(usize) {
    ...
    fn write(self: *const APIC, reg: usize, value: u32) void {
        const val: *u32 = @ptrFromInt(self.base + reg);
        val.* = value;
    }

    fn read(self: *const APIC, reg: usize) void {
        const val: *u32 = @ptrFromInt(self.base + reg);
        return val.*;
    }
};
...
fn get_lapic() APIC {
    ...
    return APIC{.base = kernel.mem.physical_to_virtual(msr.read(APIC_BASE_MSR_REG))};
}
